...
import style from "./InDepthSail.module.scss";

import clsx from "clsx";
import Progress from "react-progressbar";
import parse from "html-react-parser";

import DataJson from "../indepth-analysis/_Sail.json";
import { titleCase } from "utils/helper";

export default function InDepthSail({ title, category, color, score, icons }) {
    ...
    <div className={style.wrapper}>
      ...
      <div className={style.section}>
        ...
        <div className={style.analysis}>
          {data.critical_analysis.intro && (
            <div className={style.introText}>{parse(data.critical_analysis.intro[score])}</div>
          )}
          <ul>
      ...
      {data.improvement_strats && (
        <div className={style.section}>
          <div className={style.sectionTitle} style={{ color: color }}>
            Improvement Strategies
          </div>
          <div className={style.improv}>
            <div className={style.introText}>{parse(data.improvement_strats.intro[score])}</div>
            <ul>
