...
export default function GetInTouch() {
  ...
  const [apiResponse, setApiResponse] = useState({ isActive: false, isError: false, message: "" });
  ...
  function handleSubmitForm(e) {
    ...
    try {
      ...
      postRequest(`/web/contact`, formData, { noAuth: true })
        .then((resp) => {
          setApiResponse({ isActive: true, isError: false, message: "Thank you for contacting us." });
        })
        .catch((err) => {
          console.log(err);
          setApiResponse({ isActive: true, isError: true, message: "An error occured. Please try again" });
        })
        .finally(() => {
